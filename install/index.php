...
IncludeModuleLangFile(__FILE__);
	Class ipdesign_k1785 extends CModule
	{
		...
		function UnInstallDB($arParams = array())
		{
			...
			UnRegisterModuleDependences("main", "OnBeforeProlog", "main", "", "", "/modules/ipdesign.k1785/itdiscount.php");
			return true;
		}
		...
		function InstallFiles($arParams = array())
		{
			...
			if (is_dir($p = $_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/'.self::MODULE_ID.'/install/agent'))
			{
				if ($dir = opendir($p))
				{
					while (false !== $item = readdir($dir))
					{
						if ($item == '..' || $item == '.')
							continue;
						CopyDirFiles($p.'/'.$item, $_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/agents/'.$item, $ReWrite = True, $Recursive = True);
					}
					closedir($dir);
				}
			}
